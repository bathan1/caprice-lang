...
let owns : 'k Theory.literal -> bool =
  let is_euf_term : type a k. (a, k) Formula.t -> bool =
    ...
    | _ -> false
  in
  let owns_atom : 'k Theory.atom -> bool =
    function
    | Theory.Predicate (Binop.Equal, l, r) ->
        is_euf_term l && is_euf_term r
...
let uf_of_eq_lits (eq_lits : 'k eq_lit list) : UF.t =
  let uf = UF.create () in
...
let implied_equalities (formula : 'k Theory.literal list) : (Theory.Shared.t * Theory.Shared.t) list =
  ...
  if List.length eq_lits <> List.length formula then
    ...
    let uf = uf_of_eq_lits eq_lits in
...
let disequalities (formula : 'k Theory.literal list)
  ...
  let eq_lits =
    List.filter_map eq_lit_of_theory_lit formula
  in
  disequalities_from_lits eq_lits
    ...
    | None ->
        match
          List.find_opt
            ...
            eq_lits
        with
        | Some (Neq (_, _, original_lit)) ->
            Theory.Theory_unsat [original_lit]
        | Some (Eq _) ->
            assert false
        | None ->
            Theory.Theory_sat (model_from_uf uf)
