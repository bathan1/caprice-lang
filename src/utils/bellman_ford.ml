...
module Make (Node : Baby.OrderedType) = struct
  ...
  type pred = { edge : edge ; tail : Node.t }

  type paths = distance:(Node.t, int) Hashtbl.t * predecessor:(Node.t, pred) Hashtbl.t
  ...
  let relax_distances (num_nodes : int) (edges : edge list) (state : loop) (i : int)
    : [ `Continue of loop
      | `Stop of paths
      ] =
  ...
  let init_distance (src : Node.t) (num_nodes : int) =
    let distance = Hashtbl.create num_nodes in
    Hashtbl.add distance src 0;
    distance

  let find_paths ~(src : Node.t) (num_nodes : int) (edges : edge list) =
    let distance = init_distance src num_nodes in
    let predecessor = Hashtbl.create num_nodes in
  ...
  let find_cycle_edge (distance : ('node, int) Hashtbl.t) (edges : edge list) =
    List.find_map
  ...
  let count_num_nodes ~(src : Node.t) (edges : edge list) =
    edges
    |> List.fold_left
        (fun acc (from_, to_, _) ->
          acc
  ...
  let find_paths_or_cycle ~(src : Node.t) (edges : edge list) =
    let num_nodes = count_num_nodes ~src edges in
...
let bellman_ford
  ...
  ~(src : node)
  (edges : (node * node * int) list)
  : [ `No_negative_cycle of (node * int) list
    | `Negative_cycle of (node * node * int) list
    ] =
  let module NodeBellmanFord = Make (Node) in
  match NodeBellmanFord.find_paths_or_cycle ~src edges with
